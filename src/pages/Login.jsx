...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[4rem] p-12 relative z-10 shadow-2xl">
        <div className="text-center mb-12">
          <div className="w-20 h-20 bg-red-600 rounded-[2rem] flex items-center justify-center text-white font-black italic text-4xl shadow-2xl shadow-red-600/30 mx-auto mb-6 transform -rotate-12">
            {tenant?.name?.[0] || 'E'}
          </div>
          <h1 className="text-3xl font-black text-white italic tracking-tighter uppercase">
            {tenant?.name || 'Espeto Fácil'}
          </h1>
          <p className="text-slate-500 text-xs mt-2">Acesso exclusivo para colaboradores</p>
        </div>
        ...
        <p className="mt-12 text-center text-[10px] font-black uppercase tracking-widest text-slate-600 opacity-50">
          Espeto Fácil • Powered by i9tech
        </p>
      </div>
    </div>
  );
